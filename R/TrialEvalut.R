...
TE1<-function(df,y,rep, gen, trials, accuracy){
  ...
  for(i in 1:Ntrials){
    ...
    results$Vg[i]<-round(lme4::VarCorr(modelo)[[1]][1],5)
    results$Vres[i]<-round(lme4::sigma(modelo)^2,5)


    results$Vf[i]<-(results$Vg[i]+results$Vres[i])
    results$h2[i]<-round(results$Vg[i]/(results$Vg[i]+(results$Vres[i]/r)),4)
    results$mean[i]<-fixef(modelo)[[1]]
    results$CVg[i]<-round(100*(sqrt(results$Vg[i])/results$mean[i]),3)
    results$CVe[i]<-round(100*(sqrt(results$Vf[i])/results$m[i]),3)
    results$rgg[i]<-round(sqrt(1-(1/(1+r*(results$CVg[i]/results$CVe[i])^2))),3)
    results$Diagnostic[i]<-ifelse(results$rgg[i] < accuracy,"Remove","-")
  }
